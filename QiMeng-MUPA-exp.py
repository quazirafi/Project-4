#!/usr/bin/env python3
# Works for: 2× A100 40GB with QLoRA (4-bit) + LoRA + GRPO
...
#   python train_llama3_8b_qlora_grpo.py
#
# Notes:
# - Single-process training (NO DDP). Model shards across GPUs using device_map="auto".
...
SAVE_DIR = "checkpoints/llama3-8b-qlora-grpo-qimeng-mupa-updated-full-dataset-final-second-run-2"
SAVE_TOKENIZER = True
EXPORT_DIR = "optimized_cuda_outputs_json_llama3_8b_qimeng_mupa_updated_full_dataset_final_second_run-2"
...
def _meta_speedup_from_rec(meta: Dict) -> Optional[float]:
    if not isinstance(meta, dict):
        return None
    CAND_KEYS = [
        "gt_speedup","ground_truth_speedup","speedup","avg_speedup",
        "best_speedup","cuda_speedup","oracle_speedup","speedup_gt","max_speedup",
    ]
    for k in CAND_KEYS:
        if k in meta:
            try:
...
def load_tasks_and_seeds_from_jsonl(
    ...
    for rec in _iter_jsonl(jsonl_path):
        ...
        if decision == "PARALLELIZE" and cuda_code and sp is not None and sp >= FEWSHOT_MIN_SPEEDUP:
...
def structure_reward(features: Dict[str, float]) -> float:
    x = torch.tensor(
        [features[k] for k in RANKER["feature_names"]],
        device="cuda:0"
    )
    x = (x - MEAN_f) / STD_f
...
def final_evaluate(policy: Policy, harness: ExecutionHarness, tasks: List[Task], fewshot_pool) -> None:
    ...
    # any measurable speedup with pass_rate > 0
    speedup_cases_any_correct = 0
    speedup_gt1_cases_any_correct = 0
    total_speedup_any_correct = 0.0

    # STRICT: only fully correct CUDA codes (pass all test cases)
    fully_correct_speedup_cases = 0
    fully_correct_speedup_gt1_cases = 0
    total_speedup_fully_correct = 0.0
    ...
    for task in tasks:
        ...
        if (
            ...
            speedup_cases_any_correct += 1

            if speed > 1.0:
                speedup_gt1_cases_any_correct += 1

        # STRICT average: only full-pass outputs
        if (
            metrics.decision == "PARALLELIZE"
            and metrics.gpu_medians_ms
            and metrics.cpu_medians_ms
            and metrics.pass_rate >= 1.0
        ):
            seq_t = metrics.cpu_medians_ms[0]
            gpu_t = metrics.gpu_medians_ms[0]
            speed = seq_t / max(gpu_t, 1e-6)

            total_speedup_fully_correct += speed
            fully_correct_speedup_cases += 1

            if speed > 1.0:
                fully_correct_speedup_gt1_cases += 1
                if gt_speed is not None and gt_speed > 1.0:
                    gt_and_model_speedup_cases += 1

    avg_speedup_any_correct = (
        total_speedup_any_correct / speedup_cases_any_correct
        if speedup_cases_any_correct > 0 else 0.0
    )

    avg_speedup_fully_correct = (
        total_speedup_fully_correct / fully_correct_speedup_cases
        if fully_correct_speedup_cases > 0 else 0.0
    )
    ...
    print()
    print(f"Tasks with measurable CUDA speedup (pass_rate > 0)  : {speedup_cases_any_correct}")
    print(f"Tasks with speedup > 1 (pass_rate > 0)              : {speedup_gt1_cases_any_correct}")
    print(f"Average speedup over pass_rate > 0 tasks            : {avg_speedup_any_correct:.3f}")
    print()
    print(f"Tasks with measurable CUDA speedup (pass_rate = 1)  : {fully_correct_speedup_cases}")
    print(f"Tasks with speedup > 1 (pass_rate = 1)              : {fully_correct_speedup_gt1_cases}")
    print(f"Average speedup over fully-correct tasks            : {avg_speedup_fully_correct:.3f}")
    print()
    ...
    print(
        ...
        f"model achieved >1x speedup with full correctness: {gt_and_model_speedup_cases}/{denom}"
    )
    print("===========================================================\n")
