#!/usr/bin/env python3
# Works for: 2× A100 40GB with QLoRA (4-bit) + LoRA + GRPO on CodeLlama-34B-Instruct
#
# Install (example):
...
#   python train_codellama34b_qlora_grpo.py
#
# Notes:
...
SAVE_DIR = "checkpoints/codellama-34b-qlora-grpo-50-dataset-par-features"
SAVE_TOKENIZER = True

# =========================
# 0) CONFIG
# =========================
MODEL_NAME = "codellama/CodeLlama-34b-Instruct-hf"  # <-- CodeLlama 34B Instruct
USE_4BIT_QLORA = True

# For Llama-family 4-bit, fp16 is typically the most compatible compute dtype
TRAIN_DTYPE = torch.float16

SYSTEM_PROMPT = "You are an expert CUDA C++ engineer."

# CodeLlama does NOT have Qwen-style /think toggles
USE_NO_THINK = False
...
# IMPORTANT: keep this sane for 34B
MAX_STEP_TOK = 1536
...
# Dataset path
JSONL_PATH = "output_jsonl/sft-ft-50.jsonl"
...
# Paste this into your GRPO training script (e.g., above ExecutionHarness)
# =========================
...
def _stride_wrt_threadidx_x(expr: str) -> Optional[int]:
    """
    Returns:
      - int stride (1 is best, 2/4 worse)
      - 0 if threadIdx.x not used
      - None if threadIdx.x appears in a complex way (unknown)
    """
    e = re.sub(r"\s+", "", expr)
    if "threadIdx.x" not in e:
        return 0

    # common canonical indexing
    if re.search(r"blockIdx\.x\*blockDim\.x\+threadIdx\.x", e):
        return 1

    # threadIdx.x * k or k * threadIdx.x
    ...
    # complex expression involving threadIdx.x
    if re.search(r"threadIdx\.x[<>\|\&\^\%\/]", e) or re.search(r"[<>\|\&\^\%\/]threadIdx\.x", e):
        return None

    # default assume stride-1
    return 1

def _coal_score(stride: Optional[int]) -> float:
    """
    Coalescing proxy from stride:
      stride=1 best, stride>4 bad, unknown => 0
    """
    if stride is None:
        return 0.0
    if stride == 1:
...
def _count_ops(body: str) -> int:
    """
    Very crude op counter (string-based).
    """
    b = re.sub(r"\+\+", "", body)
    b = re.sub(r"--", "", b)
...
def _parse_threads_per_block(src: str) -> Optional[int]:
    """
    Tries to guess threads-per-block from common patterns.
    """
    m = re.search(r"\bthreadsPerBlock\s*=\s*(\d+)\s*;", src)
    if m:
        return int(m.group(1))

    # <<<grid, block>>> where block is an integer literal
...
def _transfer_proxy(host_src: str) -> float:
    """
    Penalize lots of cudaMemcpy, especially inside loops.
    Reward small number of transfers.
    """
...
def extract_struct_features(cuda_src: str) -> Dict[str, float]:
    """
    Returns a feature vector with keys:
      coal, ai, occ, div, xfer, atomics, syncthreads,
      kernels, tpb, gmem_access, ops
    """
    ...
    for kname, body in kernels:
        ...
        # global memory access proxy: array subscripts
        ...
        # arithmetic intensity proxy: ops per gmem access
        ai_k = ops / float(gmem + 1e-9)
        ai_k = float(max(-1.0, min(1.0, math.tanh(0.25 * ai_k))))

        # occupancy proxy from tpb only
        if tpb is None:
            occ_k = 0.0
        else:
            if 128 <= tpb <= 256:
        ...
        # divergence proxy: penalize non-trivial ifs
        ifs = re.findall(r"\bif\s*\((.*?)\)", body, flags=re.DOTALL)
        div_k = 0.0
        for cond in ifs:
            c = re.sub(r"\s+", "", cond)
            # allow common bounds checks
            if "idx<" in c or "idx>=" in c:
                continue
            # allow if depending purely on thread/block idx
...
def _meta_speedup_from_rec(meta: Dict) -> Optional[float]:
    """Parse speedup from a meta dict."""
...
class Policy(nn.Module):
    ...
    def _build_chat_prompt(self, user_prompt: str) -> str:
        """
        CodeLlama Instruct follows Llama2-style [INST] formatting.
        We prefer tokenizer chat_template if present; otherwise fallback to [INST].
        """
        ...
        # Llama2/CodeLlama instruct fallback
        # NOTE: keep it simple and stable for masked_logprob alignment.
        return (
            "<s>[INST] <<SYS>>\n"
            f"{SYSTEM_PROMPT}\n"
            "<</SYS>>\n\n"
...
def _select_lora_targets(model, wanted: List[str]) -> List[str]:
    ...
    if not keep:
        # Conservative fallback for Llama-family
...
def build_policy() -> Policy:
    ...
    # Many Llama-family tokenizers have no pad_token by default
    ...
    if USE_4BIT_QLORA:
        bnb_config = BitsAndBytesConfig(
            ...
            bnb_4bit_compute_dtype=torch.float16,  # CodeLlama 4-bit compute
        )
